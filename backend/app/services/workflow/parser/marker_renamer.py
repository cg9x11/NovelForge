...
def rename_variable(code: str, old_name: str, new_name: str) -> str:
    ...
    logger.info(f"[Renamer] Rename variable: {old_name} -> {new_name}")
    ...
    logger.debug("[Renamer] Rename complete")
    return code
...
def _update_node_name_meta(code: str, old_name: str, new_name: str) -> str:
    ...
    marker_bare = rf'(#@node\([^\n]*\bname\s*=\s*)({re.escape(old_name)})(seconds=[,\)])'
...
def validate_rename(code: str, old_name: str, new_name: str) -> bool:
    ...
    if old_name not in node_names:
        logger.warning(f"[Renamer] Old variable not found: {old_name}")
        return False

    if new_name in node_names and new_name != old_name:
        logger.warning(f"[Renamer] New variable already exists: {new_name}")
        return False
