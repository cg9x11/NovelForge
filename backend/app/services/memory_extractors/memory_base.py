...
class StructuredCardMemoryExtractor:
    ...
    def _build_reference_block(
        ...
        *,
        ...
        if not lines:
            return None
        return f"ã€{self.spec.reference_title}ã€‘\n" + "\n".join(lines)
    ...
    async def extract(
        ...
        *,
        ...
        if not prompt:
            raise ValueError(f"æœªæ‰¾åˆ°æç¤ºè¯: {self.prompt_name}")

        system_prompt = prompt.template
        system_prompt += f"\n\nè¯·ä¸¥æ ¼æŒ‰ä»¥ä¸‹ JSON Schema è¾“å‡º:\n{self.output_model.model_json_schema()}"

        target_names, related_names = self._partition_participants(participants)

        ref_blocks: list[str] = []
        if extra_context:
            ref_blocks.append(f"ã€è¡¥å……ä¸Šä¸‹æ–‡ï¼Œä»…ä¾›å‚è€ƒï¼Œä¸è¦æœºæ¢°å¤è¿°ã€‘\n{extra_context}")
        ...
        user_prompt = (
            f"{ref_text}"
            f"å‚ä¸Žå®žä½“ä¿¡æ¯ï¼š{participant_desc}\n\n"
            f"ç« èŠ‚æ­£æ–‡å¦‚ä¸‹ï¼š\n{text}\n"
        )
        ...
        if not isinstance(result, self.output_model):
            raise ValueError(f"{self.name}å¤±è´¥ï¼šè¾“å‡ºæ ¼å¼ä¸ç¬¦åˆ {self.output_model.__name__}")
    ...
    def persist(
        ...
        *,
        ...
        card_type = session.exec(select(CardType).where(CardType.name == self.spec.card_type_name)).first()
        if not card_type:
            raise ValueError(f"æœªæ‰¾åˆ°å¡ç‰‡ç±»åž‹ï¼š{self.spec.card_type_name}")
