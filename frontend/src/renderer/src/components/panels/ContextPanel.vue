<template>
  <div class="ctx-panel">
    <div class="panel-header">
      <h3 class="panel-title">{{ t('context_panel.participantsTitle') }}</h3>
      <el-button size="small" type="primary" :loading="assembling" @click="assemble">{{ t('context_panel.refreshContext') }}</el-button>
    </div>

    <el-form label-width="70px" class="controls">
      <el-form-item :label="t('context_panel.participants')">
        <el-select v-model="localParticipants" multiple filterable allow-create default-first-option :placeholder="t('context_panel.participantsPlaceholder')" @change="onParticipantsChange">
    ...
    <div v-if="assembled" class="assembled">
      <div class="facts-structured" v-if="assembled.facts_structured">
        <div class="facts-title" v-if="Array.isArray((assembled.facts_structured as any)?.fact_summaries) && ((assembled.facts_structured as any)?.fact_summaries?.length > 0)">{{ t('context_panel.keyFacts') }}</div>
        ...
        <div class="facts-title" v-if="Array.isArray((assembled.facts_structured as any)?.relation_summaries) && ((assembled.facts_structured as any)?.relation_summaries?.length > 0)">{{ t('context_panel.relationSummary') }}</div>
        <ul class="list" v-if="Array.isArray((assembled.facts_structured as any)?.relation_summaries) && ((assembled.facts_structured as any)?.relation_summaries?.length > 0)">
          <li v-for="(r, idx) in ((assembled.facts_structured as any)?.relation_summaries as any[] || [])" :key="idx" class="relation-item">
            ...
            <div v-if="(r as any).a_to_b_addressing || (r as any).b_to_a_addressing" class="muted addressing">
              <span v-if="(r as any).a_to_b_addressing">{{ t('context_panel.aCallsB') }}?{{ (r as any).a_to_b_addressing }}</span>
              <span v-if="(r as any).b_to_a_addressing" style="margin-left:12px;">{{ t('context_panel.bCallsA') }}?{{ (r as any).b_to_a_addressing }}</span>
            </div>
            <div v-if="Array.isArray((r as any)?.recent_dialogues) && ((r as any).recent_dialogues?.length > 0)" class="muted">
              {{ t('context_panel.dialogueExamples') }}?
              <ul class="list">
              ...
              <ul class="list">
                <li v-for="(ev, i4) in ((r as any).recent_event_summaries as any[] || [])" :key="i4">
                  ...
                  <span class="badges" v-if="(ev as any).volume_number != null || (ev as any).chapter_number != null">
                    <el-tag size="small" type="info" v-if="(ev as any).volume_number != null">{{ t('context_panel.volumeShort') }}{{ (ev as any).volume_number }}</el-tag>
                    <el-tag size="small" type="info" v-if="(ev as any).chapter_number != null" style="margin-left:6px;">{{ t('context_panel.chapterShort') }}{{ (ev as any).chapter_number }}</el-tag>
                  </span>
                </li>
              </ul>
            </div>
        ...
        <div class="facts-title" v-if="Array.isArray((assembled.facts_structured as any)?.item_summaries) && ((assembled.facts_structured as any)?.item_summaries?.length > 0)">{{ t('context_panel.itemSummary') }}</div>
        <ul class="list" v-if="Array.isArray((assembled.facts_structured as any)?.item_summaries) && ((assembled.facts_structured as any)?.item_summaries?.length > 0)">
          <li v-for="(item, idx) in ((assembled.facts_structured as any)?.item_summaries as any[] || [])" :key="`item-${idx}`" class="relation-item">
            ...
            <div v-if="(item as any).current_state" class="muted">{{ t('context_panel.currentState') }}?{{ (item as any).current_state }}</div>
            <div v-if="(item as any).owner_hint" class="muted">{{ t('context_panel.ownerHint') }}?{{ (item as any).owner_hint }}</div>
            <div v-if="(item as any).power_or_effect" class="muted">{{ t('context_panel.effectOrUse') }}?{{ (item as any).power_or_effect }}</div>
            <div v-if="(item as any).constraints" class="muted">{{ t('context_panel.constraints') }}?{{ (item as any).constraints }}</div>
            <div v-if="Array.isArray((item as any)?.important_events) && ((item as any).important_events?.length > 0)" class="muted">
              {{ t('context_panel.importantEvents') }}?
              <ul class="list">
        ...
        <div class="facts-title" v-if="Array.isArray((assembled.facts_structured as any)?.concept_summaries) && ((assembled.facts_structured as any)?.concept_summaries?.length > 0)">{{ t('context_panel.conceptSummary') }}</div>
        <ul class="list" v-if="Array.isArray((assembled.facts_structured as any)?.concept_summaries) && ((assembled.facts_structured as any)?.concept_summaries?.length > 0)">
          <li v-for="(concept, idx) in ((assembled.facts_structured as any)?.concept_summaries as any[] || [])" :key="`concept-${idx}`" class="relation-item">
            ...
            <div v-if="(concept as any).rule_definition" class="muted">{{ t('context_panel.ruleDefinition') }}?{{ (concept as any).rule_definition }}</div>
            <div v-if="(concept as any).mastery_hint" class="muted">{{ t('context_panel.masteryHint') }}?{{ (concept as any).mastery_hint }}</div>
            <div v-if="(concept as any).cost" class="muted">{{ t('context_panel.cost') }}?{{ (concept as any).cost }}</div>
            <div v-if="Array.isArray((concept as any)?.known_by) && ((concept as any).known_by?.length > 0)" class="muted">{{ t('context_panel.knownBy') }}?{{ ((concept as any).known_by as string[]).join('?') }}</div>
            <div v-if="Array.isArray((concept as any)?.counter_relations) && ((concept as any).counter_relations?.length > 0)" class="muted">{{ t('context_panel.counterRelations') }}?{{ ((concept as any).counter_relations as string[]).join('?') }}</div>
          </li>
        </ul>
      </div>
      <pre class="pre" v-if="!assembled.facts_structured && assembled.facts_subgraph">{{ assembled.facts_subgraph }}</pre>
      <div v-if="!assembled.facts_structured && !assembled.facts_subgraph">{{ t('context_panel.noFacts') }}</div>
    </div>
  </div>
</template>
...
function detectTypeGroupByCard(c: CardRead): string {
  ...
  if (et === 'organization') return t('contextPanel.groups.organization')
  ...
  if (m === 'OrganizationCard') return t('contextPanel.groups.organization')

  return t('contextPanel.groups.other')
}
...
async function buildAllGroups() {
  ...
  try {
    const cards: CardRead[] = await getCardsForProject(props.projectId)
    const order = [
      t('contextPanel.groups.character'),
      t('contextPanel.groups.scene'),
      t('contextPanel.groups.organization'),
      t('contextPanel.groups.item'),
      t('contextPanel.groups.concept'),
      t('contextPanel.groups.other')
    ]
...
async function assemble() {
  try {
    ...
    ElMessage.error(t('contextPanel.assembledFailed'))
  } finally {
