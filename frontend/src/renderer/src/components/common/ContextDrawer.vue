  ...
  <el-drawer v-model="visible" :with-header="false" size="36%" append-to-body>
    <div class="drawer-wrapper">
      <div class="drawer-header">
        <h3>{{ t('context_drawer.title') }}</h3>
        <el-button text @click="visible=false">{{ t('common.close') }}</el-button>
      </div>

      <div class="section">
        <div class="slot-toolbar">
          <h4>{{ t('context_drawer.context_template') }}</h4>
        ...
        <el-input v-model="aiContext" type="textarea" :rows="8" :placeholder="t('context_drawer.placeholder')" class="context-area" :spellcheck="false" />
        <div class="chips">
          <el-tag v-for="(t, i) in tokens" :key="i" closable @close="removeToken(t)">@{{ t }}</el-tag>
        </div>
        <div class="actions">
          <el-button size="small" @click="$emit('open-selector', { kind: activeContextTemplateKind, text: aiContext })">{{ t('context_drawer.insert_reference') }}</el-button>
          <el-button size="small" type="primary" @click="apply">{{ t('context_drawer.apply_to_card') }}</el-button>
        </div>
      </div>
    </div>
  </el-drawer>
